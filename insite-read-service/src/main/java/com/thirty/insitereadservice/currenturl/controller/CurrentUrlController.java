...
public class CurrentUrlController {
    ...
    public ResponseEntity<CurrentrUrlListResDto> getCurrentUrlList(@Valid @RequestBody CurrentUrlListReqDto currentUrlListReqDto,
...
//        int memberId = JwtProcess.verifyAccessToken(jwtToken);//검증
        int memberId = 1;
        CurrentrUrlListResDto currentrUrlListResDto = currentUrlService.getCurrentUrlList(currentUrlListReqDto,memberId);
        return new ResponseEntity<>(currentrUrlListResDto, HttpStatus.OK);
    }
}
