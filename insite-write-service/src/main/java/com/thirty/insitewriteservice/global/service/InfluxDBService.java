	...
	public void writeDataToData(DataReqDto dataReqDto) {
		...
		try (WriteApi writeApi = influxDBClient.getWriteApi()) {
			Point point = Point.measurement("data")
				...
				.addTag("activityId", getOrUpdateActivityId(dataReqDto.getCookieId()))
				.time(Instant.now(), WritePrecision.MS);
			writeApi.writePoint(bucket, org, point);
		}
	}

	public String getOrUpdateActivityId(String cookieId) {
		QueryApi queryApi = influxDBClient.getQueryApi();

		// Construct a Flux query to get the most recent activity for the given cookieId.
		Flux query = Flux.from("insite")
				.range(Instant.now().minus(30, ChronoUnit.MINUTES)) // Check for the last activity within the last 30 minutes
				.filter(Restrictions.and(
						Restrictions.tag("cookieId").equal(cookieId)
				))
				.keep(new String[]{"activityId", "time"})
				.limit(1);

		List<FluxTable> tables = queryApi.query(query.toString());

		if (tables.isEmpty() || tables.get(0).getRecords().isEmpty()) {
			// If no records are found, return a new activityId.
			return generateNewActivityId();
		} else {
			FluxRecord lastActivity = tables.get(0).getRecords().get(0);
			return lastActivity.getValueByKey("activityId").toString();
		}
	}

	public String generateNewActivityId() {
		// Implement your logic to generate a unique activityId here.
		// This could be a random UUID, or an ID generated by another service that ensures uniqueness.
		return UUID.randomUUID().toString();
	}

// Implement the generateNewActivityId method as before.
